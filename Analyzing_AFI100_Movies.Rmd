...
datasets from IMDb to create a dataset fit for analysis
...
this reason, we decided to work with official IMDB datasets, available [here](https://datasets.imdbws.com/). Should you want to run this code, you will
need to download these files to your local machine.
...
row.mask <- as.logical(apply(base, 1, check.row, top.100.movies.titles, top.100.movies.release.years))

# filter
base.filtered <- base[row.mask,]
```

Some issues, which come from the IMDb dataset. E.T. is marked as a video game,
and there is no corresponding movie in the original set. We remove this datapoint.
We could fix manually, but decided to keep things simple, and losing one datapoint
should not change things dramatically.

In some cases, tvEpisodes or video games were released the same year. We can remove
these instances. This final dataset contains one row per movie.

```{r}
base.filtered.final <- base.filtered[base.filtered$titleType == "movie",]

# print the list of tconst of interest
...
movie.dataset %>%
  ...
  theme_light() + 
  ylab("Distribution of movie runtime (in minutes)") +
  expand_limits(y = 0)
```
...
```{r}
year.genres.overall <- dplyr::bind_rows(apply(comparison.set, 1, get.movie.genres))
```
...
```{r}
# get the top 20 most represented actors
famous.actors <-
  movie.crew.info %>%
    filter(type == "actors") %>%
    group_by(name) %>%
...
movie.dataset %>%
  ...
  ggtitle("Distribution of awards received among top 100 movies") +
  theme_light()
```
